...
15. **前后端完整对照 review(第二轮):172 个 M 文件逐一定性,追齐 23 个陈旧前端文件,失败 104→80。** 四类"组件与上游一致但周边是旧版"的静默落后:① CustomRangePanel 月导航(4d7a8bd 早期漏同步,上游测试有 `.customRangeAction` 不存在的负向守卫,fork scss 死块必炸);② PriceRulesModal 三件套旧 className markup(fork Button 与上游一致、已支持 appearance API;fork 的 IconCircleHelp 增强被上游测试直接断言 `'?'`,按 Gate 3 弃);③ 10 个测试文件旧版(上游测试针对的就是我们跑的组件,直接 checkout 追齐 = 白捡覆盖率);④ 5 个品牌图标停在上游 06-12 版(dec41c9 08-04 未同步,ProviderBrandIcon.test 由红转绿)。**方法:对每个 M 文件检查 fork 侧独有行是否含已知适配标记(testutil/PG 精度/fork 特性符号),零标记 = 可疑,逐个看 diff 定性;组件不在 M 列表而其测试在 = 测试必然陈旧,无脑 checkout。**

18. **第五轮 body 级核查:同名异体函数是 symbol 审计的最后盲区。** 对 88 个文件 262 个"同名但函数体不同"的测试函数做体级 diff:提取上游侧缺失行 → 适配模式聚类(471 行命中)→ 样板噪音过滤后剩余逐行定性。真实收获 3 个:**① 放宽版断言**(usage_range_test 的 400-or-409 二选一 + 丢失用例,parser 已 0 diff 时精确断言直接可承受);**② 丢失的端点守卫**(usage_analysis_test 的 latency_diagnostics 独立端点断言);**③ 旧 parser 时代的日期口径残留**(usage_filter_test 停在 fork 365d 适配日期,parser 统一后应回上游体 + 重放 fork-only 测试)。**方法论:body 级审计用脚本提函数体(归一化注释空白)双方对比,再用"上游侧缺失行聚类"把 1400+ 行噪音压到个位数候选;逐行看完剩余候选。这是 symbol 审计之后的最后一层,两轮配合后测试文件的静默落后已全覆盖。另:全量 vitest 偶发单文件失败但隔离全过 = 负载 flaky,与后端远程 PG 时序同类,先隔离复跑再定性。**

17. **逐文件历史未同步清查(第四轮):全量 symbol 审计抓 ~30 个缺失函数,分三类处置。** 方法:对全部 149 个 M 测试文件跑 `diff <(git show upstream/main:f | grep '^func') <(grep '^func' f)`,缺失即候选;再用**生产符号存在性**二分:生产有 → 纯测试同步(checkout+PG 适配);生产无 → 特性级未同步,深挖。本轮唯一特性级:**latency 365 天** —— fork 的 30 天 guard 是 f3b55ec(rollup 重构)之前的旧口径,fork 同步了该 commit 仓储层却漏了 API 层 guard 删除(跨文件 commit 部分同步的典型案例);上游由共享 parser 统一 365 上限,rollup 日报表本就保留 365 天 → 删 guard 回归上游,invariants 同步移除过时符号。**教训:(a) "fork-unique 符号"不等于"fork 要保" —— invariants 清单里可能躺着已过时的 fork guard,上游演进后它从特性变债务,每次大 review 要反向问"这个 guard 上游现在怎么做";(b) per-file symbol 审计对 helper 函数有假阳(helper 挪到共享文件),要用包级 grep 复核;(c) app 启动同步 catch-up 与上游 runner-wake 是记录在案的架构分歧,上游对应测试定性 SKIP。**

16. **⚠️ "stub 先发通道再写库"测试模式在远程 PG 上是可见性竞态(30% 失败率)。** timer boundary 测试的 writer stub 先向 `writes` 通道投递观测副本、再执行真实落库;测试收到副本后仅等 20ms 就读库。SQLite 写 µs 级完成的隐含假设在远程 PG(5 往返 ~100ms+)下不成立,且负载越高失败率越高(易误判为"墙钟敏感"或"负载 flake" —— 本文 #9 曾误扩窗 1s→10s 未根治,循环 20 次实测 6 失败才定位)。**修复:收到批次后轮询等待 DB 可见(deadline 10s)。判别法:失败率与负载正相关 + 单跑偶过 = 竞态;固定失败 = 确定性 bug;与时辰相关 = 墙钟。任何 stub 内"发通道 → 真实写"两步的测试都要轮询读。**
