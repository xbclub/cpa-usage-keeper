...
13. **已知测试覆盖债(~45 个上游测试文件未同步,待专项清理):** 全树对比暴露 fork 缺上游常规测试:`repository/test/` 的 `*_pricing_rules_test.go` 集群(~20 文件)、`quota/test/` ×4、`service/test/` ×4(pricing_catalog_mutation 等)、`app/test/` ×3、`api/test/pricing_rules_test.go`、`config/test/listen_host_test.go`、`poller/test/usage_aggregation_runner_shared_test.go`、前端 `credentials/test/` ×5(QuotaResetAction 等);另有 rename-split:fork 独有 `pricing_test_helpers_test.go`(复数,5 处)vs 上游 `pricing_test_helper_test.go`(单数)。**这些是历次选择性 merge 的累积遗漏,非本次引入;补齐需逐文件 PG 适配,属独立 test-sync 专项。**
