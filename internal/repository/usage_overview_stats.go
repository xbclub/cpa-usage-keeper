...
// applyUsageOverviewHourlyStats 分批写入小时聚合行，复用批量参数数量保护。
...
// applyUsageOverviewDailyStats 分批写入天聚合行，复用批量参数数量保护。
...
// applyUsageOverviewHealthStats 分批写入 health 聚合行，复用批量参数数量保护。
