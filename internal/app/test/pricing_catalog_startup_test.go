...
func TestPricingCatalogStartupFailsWhenPersistedSnapshotIsInvalid(t *testing.T) {
	...
	seedDB, err := repository.OpenDatabase(config.Config{DatabaseURL: databaseURL})
	if err != nil {
	...
	logDir := t.TempDir()
	cfg := pricingCatalogStartupConfig(databaseURL)
	...
	verificationDB, openErr := repository.OpenDatabase(config.Config{DatabaseURL: databaseURL})
	if openErr != nil {
...
func pricingCatalogStartupConfig(databaseURL string) config.Config {
